# $env:PATH += ";../../tools/avr/bin/"

# replace in ../compress/compress.fsproj netcoreapp5.0 with netcoreapp6.0
# Define the file path
$filePath = "../compress/compress.fsproj"

# Read the file content
$fileContent = Get-Content -Path $filePath

# Replace the target framework version
$fileContent = $fileContent -replace "netcoreapp5.0", "netcoreapp6.0"
...
function compile($f) {
  ...
  avr-g++ `
    ...
    -DAVR `
    -std=gnu++1z `
    -Os --short-enums -fpermissive -fshort-enums $f -c -o "$m/$n.o"
  avr-objcopy -O binary -j .data "$m/$n.o" "$m/$n.hex"
  cd ../compress
...
function gen_h($f) {
  ...
  $syms = @()
  $typs = @{}
  foreach($line in $(avr-objdump -x "$n.o")) {
