...
bool MCLActions::get_dev_slot_info(uint8_t slot_number, DeviceSlotInfo* pinfo) {
  GridDeviceTrack *p = get_grid_dev_track(slot_number, &pinfo->track_idx, &pinfo->dev_idx);
  pinfo->grid_idx = get_grid_idx(slot_number);
  if (p) {
    pinfo->track_type = p->track_type;
    pinfo->group_type = p->group_type;
    pinfo->seq_track = p->seq_track;
    return true;
  }
  pinfo->track_type = 255;
  pinfo->seq_track = nullptr;
  return false;
}
...
void MCLActions::store_tracks_in_mem(int column, int row,
  ...
  uint8_t i = 0;
  DeviceSlotInfo dev_info;

  for (i = 0; i < NUM_SLOTS; i++) {
    if (slot_select_array[i] > 0 && get_dev_slot_info(i, &dev_info)) {
      save_dev_tracks[dev_info.dev_idx] = true;
    }
  }
  ...
  for (i = 0; i < NUM_SLOTS; i++) {
    if (slot_select_array[i] > 0) {
      ...
      bool dev_slot_ok = get_dev_slot_info(i, &dev_info);
      online = (elektron_devs[dev_info.dev_idx] != nullptr);
      // If save_dev_tracks[dev_idx] turns false, it means getBlockingKit
      // has failed, so we just skip this device.
      if (!save_dev_tracks[dev_info.dev_idx]) {
        continue;
      }

      if (dev_slot_ok) {
        proj.select_grid(dev_info.grid_idx);

        // Preserve existing chain settings before save.
        if (row_headers[dev_info.grid_idx].track_type[dev_info.track_idx] != EMPTY_TRACK_TYPE) {
          grid_track.load_from_grid(dev_info.track_idx, row);
          memcpy(&empty_track.chain, &grid_track.chain,sizeof(GridChain));
        } else {
          empty_track.chain.init(row);
        }
        auto pdevice_track =
            ((DeviceTrack *)&empty_track)->init_track_type(dev_info.track_type);
        pdevice_track->store_in_grid(dev_info.track_idx, grid_page.getRow(), dev_info.seq_track,
                                     merge, online);
        row_headers[dev_info.grid_idx].update_model(
            dev_info.track_idx, pdevice_track->get_model(), dev_info.track_type);
      }
    }
  }
...
void MCLActions::prepare_next_chain(int row, uint8_t *slot_select_array) {
  ...
  for (uint8_t n = 0; n < NUM_SLOTS; ++n) {
    ...
    if ((slot_select_array[n] == 0) || !get_dev_slot_info(n, &dev_info)) {
    ...
    proj.select_grid(dev_info.grid_idx);
    auto device_track = empty_track.load_from_grid(dev_info.track_idx, row);
    if (device_track == nullptr || device_track->active != dev_info.track_type) {
      empty_track.clear();
      device_track = device_track->init_track_type(dev_info.track_type);
      send_machine[n] = 1;
    } else {
      send_machine[n] = 0;
    }
    device_track->store_in_mem(dev_info.track_idx);
  }
...
void MCLActions::send_tracks_to_devices(uint8_t *slot_select_array) {
  ...
  for (uint8_t i = 0; i < NUM_SLOTS; i++) {
    ...
    if (!select_array[i] || !get_dev_slot_info(i, &dev_info)) {
      // Ignore slots that are not device supported.
      continue;
    }

    mute_states[i] = dev_info.seq_track->mute_state;
    dev_info.seq_track->mute_state = SEQ_MUTE_ON;
    ...
    auto *ptrack = empty_track.load_from_grid(dev_info.track_idx, grid_page.getRow());
    ...
    if (ptrack->active != dev_info.track_type) {
      ptrack->init_track_type(dev_info.track_type);
      ptrack->transition_clear(dev_info.track_idx, dev_info.seq_track);
    } else {
      ptrack->load_immediate(dev_info.track_idx, dev_info.seq_track);
      if (dev_info.track_type != 255) {
        send_masks[i] = 1;
      }
    }
  }
  ...
  for (uint8_t i = 0; i < NUM_SLOTS; ++i) {
    if (get_dev_slot_info(i, &dev_info)) {
      dev_info.seq_track->mute_state = mute_states[i];
    }
  }
  ...
  for (uint8_t n = 0; n < NUM_SLOTS; n++) {
    if (select_array[n] && grid_page.active_slots[n] >= 0 && get_dev_slot_info(n, &dev_info)) {
      transition_level[n] = 0;
      next_transitions[n] =
          MidiClock.div16th_counter -
          (dev_info.seq_track->step_count * dev_info.seq_track->get_speed_multiplier());
...
void MCLActions::cache_next_tracks(uint8_t *slot_select_array,
  ...
  for (uint8_t n = 0; n < NUM_SLOTS; n++) {
    if (slot_select_array[n] > 0) {
      ...
      if (!get_dev_slot_info(n, &dev_info)) {
        continue;
      }
      ...
      auto *ptrack = empty_track->load_from_grid(dev_info.track_idx, chains[n].row);

      if (ptrack == nullptr || ptrack->active != dev_info.track_type) {
        // EMPTY_TRACK_TYPE
        empty_track->clear();
        empty_track->init_track_type(dev_info.track_type);
        send_machine[n] = 1;
      } else {
        auto *pmem_track = empty_track2->load_from_mem(dev_info.track_idx, dev_info.track_type);
      ...
      ptrack->store_in_mem(dev_info.track_idx);
    }
  }
...
void MCLActions::calc_next_slot_transition(uint8_t n) {
  ...
  len = (float)chains[n].loops * l * (float)dev_info.seq_track->get_speed_multiplier();
  while (len < 4) {
    if (len < 1) {
      len = 4;
...
void MCLActions::calc_latency(DeviceTrack *empty_track) {
  ...
  for (uint8_t n = 0; n < NUM_SLOTS; n++) {
    ...
    if (next_transitions[n] == next_transition) {
      ...
      if (send_machine[n] == 0) {
        auto *ptrack = empty_track->load_from_mem(dev_info.track_idx, dev_info.track_type);
        if (ptrack == nullptr || !ptrack->is_active() ||
            dev_info.track_type != ptrack->active) {
          continue;
        }
      dev_latency[dev_info.dev_idx].latency += ptrack->calc_latency(n);
      }
      send_dev[dev_info.dev_idx] = true;
    }
  }
