...
struct chunk_t {
  ...
  uint32_t total_len() const {
    return chunk_size + sizeof(chunk_t);
  }
...
struct fmtchunk_t: public chunk_t {
  ...
  void init(const uint16_t &nchannel, const uint32_t &smplrate, const uint8_t &bitrate) {
...
struct smplchunk_t: public chunk_t {
  ...
  void init(const fmtchunk_t& fmt, uint8_t SDS_loop_type, uint32_t SDS_loop_start, uint32_t SDS_loop_end) {
  ...
  void to_sds(const fmtchunk_t& fmt, uint8_t &SDS_loop_type, uint32_t &SDS_loop_start, uint32_t &SDS_loop_end) {
...
public:
  ...
  bool open(char *file_name, bool write = false, uint16_t numChannels = 1,
            uint32_t sampleRate = 44100, uint8_t bitRate = 16, bool loop = false);
  ...
  bool read_samples(void *data, uint32_t num_samples,
                    uint32_t sample_offset = 0, uint8_t channel = 0, uint8_t num_of_channels = 1);
