...
class WavHeader {
  ...
  init_header(uint16_t numChannels_, uint32_t sampleRate_, uint8_t bitRate_) {
...
class Wav {
  ...
  wave(char *file_name, uint16_t numChannels = 1, uint32_t sampleRate = 44100,
       uint8_t bitRate = 16) {
    wave_file.open(file_name, O_RDWR);
    if (wave_file.fileSize() == 0) {
  ...
  bool write_samples(void *data, uint32_t num_samples, uint32_t sample_offset = 0,
                     uint8_t channel = 0);
  bool read_samples(void *data, uint32_t num_samples, uint32_t sample_offset = 0,
                    uint8_t channel = 0);
};
