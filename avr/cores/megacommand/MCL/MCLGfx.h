...
public:
  ...
  MCLGIF(uint8_t *bmp, uint8_t frame_offset_, uint8_t num_of_frames_, uint8_t w_, uint8_t h_, MCLGIFDir dir_) {
    bitmap = bmp;
  ...
  uint8_t *get_next_frame() {
    ...
    if (clock_diff(last_frame_clock,slowclock) < 100) { return bmp; }
