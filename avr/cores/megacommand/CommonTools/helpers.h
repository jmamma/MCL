...
#define ROTATE_LEFT(target, length) do { target = (target >> shift) | (target << (length - shift)); } while (false)
#define ROTATE_RIGHT(target, length) do { target = (target << shift) | (target >> (length - shift)); } while (false)
