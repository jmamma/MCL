...
| High | Grid, Slot, Save and Load | `grid_init_annot.png`, `grid_populated.png`, `slot_menu.png`, `range_copy.png`, `save_to_a.png`, `load_from_a.png`, `group_select_page.png`, `load_destination.png` | Kept the annotated grid layout diagram and added a populated-slot example. Recaptured Slot Menu, range selection, Save, Load, Group Selector and Load Destination. |
...
| High | Mixer and Performance | `mixer_page_*.png`, `perf*.png` | Recaptured Mixer selector/base/performance-state preview and Performance selector/base/learn/menu/scene-lock screens. Removed stale `mixer_page_perf_lock.png`; the lock workflow is documented in text because headless attempts do not yet reproduce a readable lock-marker screen reliably. |
