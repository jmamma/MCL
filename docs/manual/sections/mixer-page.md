...
- Mute Enable for each mixer target
...
| While holding a state arrow, press **[Bank A]** | Toggle Mute Enable for the visible mixer target. When off, that target's stored mutes/fills are not applied. |
