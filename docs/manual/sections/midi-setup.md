...
| Connection | Cable |
| --- | --- |
| Machinedrum MIDI OUT -> MCL MIDI IN 1 | Sends Machinedrum keys, transport, sysex and state changes to MCL. |
| MCL MIDI OUT 1 -> Machinedrum MIDI IN | Sends sequencing, parameter, sysex and control data back to the Machinedrum. |
...
| `CONFIG > MIDI > DEVICES > GRID X > PORT` | `MIDI 1`, meaning MCL MIDI IN 1 / OUT 1 with standard 5-pin MIDI cables |
...
| Connection | Cable |
| --- | --- |
| Device MIDI OUT -> MCL MIDI IN 2 | Lets MCL receive sysex, transport or performance state where supported. |
| MCL MIDI OUT 2 -> Device MIDI IN | Lets MCL sequence and control the device. |
...
| Connection | Cable |
| --- | --- |
| Controller or keyboard MIDI OUT -> MCL MIDI IN 2 or USB | Optional live input for chromatic play, recording or controller forwarding. |
| MCL MIDI OUT 2 or USB -> External device MIDI IN | Sends notes, automation and program changes from MCL. |
