...
private:
    ...
    ALWAYS_INLINE() uint8_t read_char() {
        return uart_getc(uart_hw);
    }

    ALWAYS_INLINE() bool check_empty_tx() {
        return uart_is_writable(uart_hw);
    }
...
public:
    ...
    volatile RingBuffer<0> *rxRb;
    volatile RingBuffer<0> *txRb;
    volatile RingBuffer<0> *txRb_sidechannel;
    ...
    MidiUartClass(uart_inst_t* uart_hw, RingBuffer<0> *_rxRb = nullptr, RingBuffer<0> *_txRb = nullptr);
