...
public:
  ...
  static int32_t check_stack_space() {
    ...
    if (sp > stack_top) {
      return -1 *
             (sp - stack_top); // Return negative number indicating overflow
    }

    // If SP is below bottom, we've underflowed
    if (sp < stack_bottom) {
      return -1 *
             (stack_bottom - sp); // Return negative number indicating underflow
    }
  ...
  static void print_stack_info() {
    ...
    // Format core number and stack addresses
    sprintf(buf, "Core %d Stack: SP=0x%08lx Bottom=0x%08lx Top=0x%08lx ",
            get_core_num(), get_sp(), stack_bottom, stack_top);
    DEBUG_PRINT(buf);

    if (space < 0) {
      sprintf(buf, "OVERFLOW: %ld bytes beyond boundary!\n", -space);
    } else {
      sprintf(buf, "Free=%ld bytes (%ld%%)\n", space,
