...
bool DeviceTrackChunk::load_from_mem_chunk(uint8_t column, uint8_t chunk) {
    ...
    uint8_t* ptr = get_region() + get_track_size() * column +
                   (seq_data_chunk - reinterpret_cast<uint8_t*>(this)) +
                   chunk_size * chunk;
    memcpy_bank1(seq_data_chunk, ptr, chunk_size);
    return true;
}

bool DeviceTrackChunk::load_chunk(uint8_t* ptr, uint8_t chunk) {
    size_t chunk_size = sizeof(seq_data_chunk);
...
bool DeviceTrackChunk::load_link_from_mem(uint8_t column) {
    uint8_t* ptr = get_region() + get_track_size() * column +
                   (reinterpret_cast<uint8_t*>(&link) - reinterpret_cast<uint8_t*>(this));
    memcpy_bank1(&link, ptr, sizeof(GridTrack));
    return true;
}
