...
void opt_clear_track_handler() {
  ...
  if (opt_midi_device_capture == &MD ||
    ...
    if (opt_clear == 2) {
      ...
      oled_display.textbox_P(str);
  } else {
    if (copy) {
      opt_copy_track_handler(opt_clear);
    }
    const char *str_p = mclstr_clear_ext_track;
    if (opt_clear == 2) {
      for (uint8_t n = 0; n < mcl_seq.num_ext_tracks; n++) {
        str_p = mclstr_clear_ext_tracks;
...
void opt_copy_track_handler(uint8_t op) {
  ...
  if (opt_copy == 1) {
    if (opt_midi_device_capture == &MD) {
      if (!silent) {
        oled_display.textbox_P(mclstr_copy_track);
        MD.popup_text(4);
      }
      ...
      if (!silent) {
        ...
        oled_display.textbox_P(mclstr_copy_ext, mclstr_track);
      }
...
void opt_paste_track_handler() {
  ...
  if (opt_paste == 2) {
    ...
    if (opt_midi_device_capture == &MD) {
      if (!undo) {
        oled_display.textbox_P(mclstr_paste_md_tracks, mclstr_tracks);
        oled_display.display();
        MD.popup_text(3);
      } else {
        oled_display.textbox_P(mclstr_undo_tracks, mclstr_tracks);
        oled_display.display();
        MD.popup_text(22);
      }
  ...
  if (opt_paste == 1) {
    if (opt_midi_device_capture == &MD) {
      bool is_poly = false;
      if (!undo) {
        oled_display.textbox_P(mclstr_paste_track);
        MD.popup_text(6);
      } else {
        oled_display.textbox_P(mclstr_undo_track);
...
void opt_clear_page_handler() {
  ...
  oled_display.textbox_P(mclstr_clear_page);
  MD.popup_text(57);
...
void opt_copy_page_handler(uint8_t op) {
  ...
  if (!silent) {
    oled_display.textbox_P(mclstr_copy_page);
    MD.popup_text(54);
  }
  for (uint8_t n = 0; n < 16; n++) {
    uint8_t step = n + SeqPage::page_select * 16;
...
void opt_paste_page_handler() {
  if (opt_undo == PAGE_UNDO) {
    opt_undo = 255;
    oled_display.textbox_P(mclstr_undo_page);
    MD.popup_text(55);
  } else {
    oled_display.textbox_P(mclstr_paste_page);
    MD.popup_text(56);
  }
  for (uint8_t n = 0; n < 16; n++) {
    uint8_t step = n + SeqPage::page_select * 16;
...
void opt_clear_step_locks_handler() {
  if (opt_clear_step == 1) {
    oled_display.textbox_P(mclstr_clear_step, mclstr_locks);
    MD.popup_text(14);
  }
  for (uint8_t n = 0; n < NUM_MD_TRACKS; n++) {
    if (note_interface.is_note_on(n)) {
...
void opt_transpose_track_handler() {
  ...
  if (opt_midi_device_capture == &MD) {
    if (is_all) {
      for (uint8_t n = 0; n < NUM_MD_TRACKS; n++) {
        mcl_seq.md_tracks[n].transpose(transpose_value);
      }
    } else {
      mcl_seq.md_tracks[last_md_track].transpose(transpose_value);
    }
  }
#ifdef EXT_TRACKS
  else {
    if (is_all) {
      for (uint8_t n = 0; n < NUM_EXT_TRACKS; n++) {
        mcl_seq.ext_tracks[n].transpose(transpose_value);
      }
    } else {
      mcl_seq.ext_tracks[last_ext_track].transpose(transpose_value);
    }
  }
#endif
 // opt_transpose = 12;
