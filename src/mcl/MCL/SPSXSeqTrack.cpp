...
// ============================================================================
// SPSXSeqTrackBase — Timing Methods
// ============================================================================

uint16_t SPSXSeqTrackBase::get_ticks_per_step(uint8_t speed_) {
    uint16_t tps;
    switch (speed_) {
    default:
...
// ============================================================================
// SPSXSeqTrackCond — Conditional Evaluation
// ============================================================================

void SPSXSeqTrackCond::record_trig_result(bool fired) {
    prev_trig_fired = fired;
    if (seq_class) {
        if (fired) {
            SPSX_SET_BIT16(seq_class->neighbor_trig_mask, track_number);
        } else {
            SPSX_CLEAR_BIT16(seq_class->neighbor_trig_mask, track_number);
        }
    }
}

bool SPSXSeqTrackCond::neighbor_fired() const {
    if (!seq_class || track_number == 0) return false;
    return SPSX_IS_BIT_SET16(seq_class->neighbor_trig_mask, track_number - 1);
}

bool SPSXSeqTrackCond::conditional(uint8_t condition) {
    // Percentage-based (0-7)
    if (condition <= SPSX_COND_10PCT) {
        static const uint8_t pct_thresholds[] = {
            255, 230, 192, 169, 128, 84, 64, 26
        };
        if (condition == SPSX_COND_100PCT) return true;
        return spsx_get_random_byte() <= pct_thresholds[condition];
    }

    // Special conditions (8-16)
    switch (condition) {
    case SPSX_COND_ONESHOT:   return true; // handled in trig_conditional
    case SPSX_COND_FIRST:     return first_run;
    case SPSX_COND_NOT_FIRST: return !first_run;
    case SPSX_COND_FILL:
        return seq_class && SPSX_IS_BIT_SET16(seq_class->fill_mask, track_number);
    case SPSX_COND_NOT_FILL:
        return !seq_class || !SPSX_IS_BIT_SET16(seq_class->fill_mask, track_number);
    case SPSX_COND_PRE:       return prev_trig_fired;
    case SPSX_COND_NOT_PRE:   return !prev_trig_fired;
    case SPSX_COND_NEI:       return neighbor_fired();
    case SPSX_COND_NOT_NEI:   return !neighbor_fired();
    default: break;
    }

    // Iteration ratios X:Y (17-51)
    if (condition >= SPSX_COND_ITER_BASE && condition <= SPSX_COND_ITER_MAX) {
        uint8_t x, y;
        if (spsx_cond_iter_decode(condition, x, y)) {
...
// ============================================================================
// SPSXSeqSlideTrack — Slide/Glide Implementation
// ============================================================================

void SPSXSeqSlideTrack::on_slide_dispatch_begin(uint8_t) {}
void SPSXSeqSlideTrack::on_slide_dispatch_end() {}

void SPSXSeqSlideTrack::dispatch_slide_value(uint8_t param, uint8_t value, uint8_t channel) {
    if (port) {
        port->sendCC(channel, param, value);
    }
}

void SPSXSeqSlideTrack::prepare_slide(uint8_t lock_idx, int16_t x0, int16_t x1, int8_t y0, int8_t y1) {
    uint8_t c = lock_idx;
...
void SPSXSeqSlideTrack::send_slides(volatile uint8_t *locks_params, uint8_t channel) {
